...
```{r project.setup, echo=TRUE, message=FALSE, results='hide'}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.cap='2018 Monthly Ozone Levels', fig.height=3}

epa_data$Date <- ymd(epa_data$Date)

epa_data.2018 <- epa_data %>%
  ...
  ggplot(epa_data.2018, aes(x = Month, y = Ozone)) +
    geom_boxplot() +
  labs(
    title = '2018 Monthly Ozone Levels',
    x = 'Month',
...
epa_data.2019 <- epa_data %>%
  ...
  ggplot(epa_data.2019, aes(x = Month, y = Ozone)) +
    geom_boxplot() +
  labs(
    title = '2019 Monthly Ozone Levels',
    x = 'Month',
    y = 'Ozone Concentration (µg/m³)'
    )
```
...
## Git Repository
https://github.com/emilesflynn/EDE_Fall2025
