...
> Answer: The console will run the code, meaning that it will check the library (open source) for your specific package named within the parenthesis, and confirm if the package is installed into your R repository. It's important to watch the console for an error in loading a package, in case you need to fix the code or your computer settings to enable the package to work successfully. After downloading the package, remember to "#comment" the package because it can interfere with knitting your markdown R document. 

---

### Establishing the working directory
Console: >getwd() "get working directory" (relative path) OR >setwd() "set your directory to any path (absolute path)
...
#file location is the "absolute pathway"(P:///.csv) on your computer VS "relative path" (C:.//.csv) is pointing to a file considering where you are right now (ex. start at working directory to someplace else)
...
# What happens if we don't assign a name to our imported dataset? The console will just confirm that R can access the file from your work directory or computer files. If we assign a name, then the selected raw data file will opened/shown in the CONSOLE and organized in the ENVIRONMENT tab.
read.csv(here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),stringsAsFactors = TRUE)
#^shortcut
#CONSOLE: read.csv('TAB'), then copy & paste into SOURCE tab
...
```{r Exploring data}
##View(USGS.flow.data) #COMMENT OUT THIS WHEN KNITTING!!!
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data) #what type of variable is the (variable)
colnames(USGS.flow.data) #column names, only "names" works too
names(USGS.flow.data)

# Rename columns (vector with a bunch of strings, names found in metadata!)
...
str(USGS.flow.data) #structure, show size of dataframe (#obs, #rows, understand data types)
##glimpse(USGS.flow.data) #^same as str, tidyverse version
dim(USGS.flow.data) #dimension (#rows, columns)
length(USGS.flow.data) #of columns

head(USGS.flow.data) #provide (default) first 5 records of each column
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #(3000:3005)=matrix notation to look at selected rows AND selected rows "c(3,8,14)

class(USGS.flow.data$datetime) #specific columns
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)  #could point to column only with $, summary stats only for numeric columns
...
```{r Finding NAs}
summary(USGS.flow.data$discharge.mean) #shows that 51000 rows are missing "NA"s
...
> Answer: choose to omit columns or row, leave the data as is, assign a value

```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #NA omit command
dim(USGS.flow.data)
...
```{r}
USGS.flow.data$site_no <- as.factor(USGS.flow.data$site_no) #make site number, a interger, into a factor variable type
str(USGS.flow.data)
```
...
USGS.flow.data2  <- read.csv(
  file = here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),
  stringsAsFactors = TRUE) 
  colClasses = c('site_no'='factor') #override R default column variable type settings to your choosen one
```
...
```{r Date formats}
#default R settings for date is a character, we want to specify a numerical variable type so that R can calulate or develop codes for Time Series and etc.
help(as.Date)
...
today <- Sys.Date()
format(today, format = "%B") #"%B" shows the month associated in the choosen date format
#format(today, format = "")
#format(today, format = "")
#format(today, format = "")

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") 
#"as.Date" converts a string to a date object, to all strings, and then specify format wanted
#note: the first data record, "USGS.flow.data$datetime[1]" is [1] "2027-10-01" which is not a reasonable record (future???)
```
...
```{r Fixing dates}
# 
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d") #convert back into a string
