...
data_source <- paste0(                                   ##data source is online
  'https://services.nconemap.gov/secure/rest/services/', ##make URL in more than one line because it reads better, also 
  'NC1Map_Hazardous_Waste/MapServer/4/',                 ##new term for myself (space 0 - no spaces)
...
##^USF&WL website > biota > "turtles" > (select) 2023 Sea Turtle Nesting Survey TAFB survey > view API resources > GeoJSON file
...
```{r cluster map}
#Create the map
turtle_map <- leaflet(sea_turtles) %>%  ##leaflet("name of journal"")
  addTiles() %>% 
  addMarkers(
...
##export and save as a webpage to share with others
##data websites are called "feature service directory" on sites that have several different download options online
```
