...
epa_pm25_sites_sf <- read_csv(here('Data/Raw/EPAair_PM25_NC2018_raw.csv')) %>% 
  ...
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269) ###NAD83
...
###sf files include info on shape, projection, data, etc information
###vs. GeoJSOn is a single, text-based file (more user friendly)
counties_sf<- st_read(here('Data/Spatial/cb_2018_us_county_20m.shp')) %>% 
  filter(STATEFP == 37) #Filter for just NC Counties, with 100 counties (therefore 100 obs)
...
st_bbox(counties_sf) ###bounty box creates a rectangles around our dataset
...
###E = estimate, M = Margin of Error, "POV150" 150% of poverty line, "RPL_THEME1" socioeconomic ranking (shown above markdown text)

#Check structure
glimpse(svi2020_nc_raw)

#Join the SVI attributes to the county spatial features "MERGE" (base R function version)
...
#Tidyverse version of the join "LEFT_JOIN" (same output as merge)
...
#View with mapview, visual analysis
...
#view with ggplot
ggplot() + 
  geom_sf(data=counties_sf_join,aes(fill = RPL_THEME1),alpha=0.5) + ###alpha = transparency
...
wind_raw <- read.csv(
  ...
  colClasses = c('HUC_12'='factor') ###as factor
  ) %>%  
  mutate(huc_8 = substr(HUC_12, 0, 8)) %>%  #first 8 digits
  group_by(huc_8) %>% 
  summarize(
...
state_sf <- counties_sf %>% 
  group_by('STATEFP') %>%       ###combine 100 counties into 1 state boundary
  summarize(ALAND = sum(ALAND)) ###sum of area of land
...
mapview(huc8_sf, zcol="DWQ_Basin", legend = FALSE)
#List the unique values in the DWQ_Basin field
unique(huc8_sf$DWQ_Basin)

#Summarize on DWQ Basin value, dissolve boundaries
huc2_sf <- huc8_sf %>% 
  group_by(DWQ_Basin) %>% 
  summarize(
    ACRES = sum(ACRES)
...
###"st_transform" translates one coordinate reference system to another)
...
neuse_intersect <- counties_sf_utm %>%  
  st_filter(neuse_sf, .predicate = st_intersects) ###predicate shows what function

#View the result
myMap + mapview(neuse_intersect, alpha.regions = 0) ###shows what counties intersect with HUC Neuse

#Actually intersect the counties (not just select those that intersect)
neuse_counties_sf <- neuse_sf %>% st_intersection(counties_sf_utm) ###cookie cutter, takes counties and mask it to the polygon HUC Neuse
...
neuse_counties_sf <- neuse_counties_sf %>% 
  mutate(Area_m2 = as.numeric(st_area(neuse_counties_sf$geometry))) ###calculate area with st_area
...
huc8_sf_utm %>% st_filter(triCo) %>% mapview() ###default is intersect under filter, unless changed with the precident function to select another tool
...
myMap
###manipulated the original geometeries (polygons) to a different one

#Buffer the centroids outward 2km and add them to our
triCo_centroids_2km <- st_buffer(triCo_centroids, 2000) ###buffer on geometric objects centroids
myMap <- myMap + geom_sf(data = triCo_centroids_2km, color = 'orange', fill=NA)
myMap

#Buffer the counties inward 2km
triCo_in2km <- st_buffer(triCo, -2000) ### buffer inwards from county boundaries
...
triCo_centroids_chull <- triCo_centroids %>% 
  st_union() %>% ###combine and not dissolve
  st_convex_hull()
...
triCo_centroids_voronoi <- triCo_centroids %>%    ###ex. rain gauges, way to extrapolate areas
  st_union() %>%                                  ##puts in single feature
  st_voronoi()
...
###True is the feature that intersects

#Filter the counties dataset using the boolean mask
selCounties <- counties_sf_utm[resultMask,] ###return the one feature that is true

#Map the results
mapView(counties_sf[resultMask,]) ###durham county!
...
# Convert all to WGS84 (crs=4326) ###back to a geographic coordinate sys
...
leaflet() %>% addTiles() %>% ###basic leaflet map
  addPolygons(data=counties_WGS84,weight=1,color='red') %>% ###adds counties
  addPolygons(data=huc8s_WGS84,weight=1)
###creates a web map that we need to manually add attributes
```
...
leaflet() %>% 
  addProviderTiles(providers$Esri.NatGeoWorldMap) %>%  ###changes world map
