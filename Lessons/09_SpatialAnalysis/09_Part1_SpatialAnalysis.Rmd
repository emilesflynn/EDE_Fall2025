...
###similar to read_csv function but for spatial data format, shows metadata in console with geodetic CRS "coordinate reference system"

#Explore as a typical dataframe
class(GasPlants)
glimpse(GasPlants)
str(GasPlants$geometry) ###there is a geometry column in the dataset

#Explore as a spatial dataframe...
#View using sf plot
plot(GasPlants['status'],pch=16) ###select a column "status", A = active, I = inactive

#View using ggplot
ggplot(data=GasPlants) + geom_sf(aes(color=status)) ###geom_sf in ggplot

#View using mapview
mapview(GasPlants, zcol='status') ###zcol = change colors based on status
...
-   **Simple feature object (sf)**: the entire "spatial dataframe"
-   **Simple feature**: one row in the sf collection
-   **Simple feature column (sfc)**: the column containing geometries or "geom"
...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    mean_Ozone = mean(Ozone, na.rm=T), #What does "na.rm=T" do? Is it necessary? Ignore records with missing data "N/A"
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(
    coords = c('mean_Lng','mean_Lat'), ###creates a geometry column, long E/W x-values, lat flat N/S y-values
    crs=4326
    )

#Save locally as a shapefile 
st_write(
  EPAair.sf, ###name of spatial dataframe to save to files
  here("./Data/Processed/EPAair_sites.shp"),  ###where to save in files
  driver='ESRI Shapefile'
  )

#View the sf object using the methods above...
mapview(EPAair.sf, cex="mean_PM25") ###cex sizes attributes
```
...
mapview(sfg_GraingerHall) ###will plot but no idea on earth

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) ###add crs
class(sfc_GraingerHall)
mapview(sfc_GraingerHall)

#Upgrade to a spatial dataframe
sf_GraingerHall <- st_as_sf(sfc_GraingerHall)
sf_GraingerHall$Name = 'Grainger Hall' ###add attributes to the spatial dataframe
...
the_buffer <- sf_GraingerHall %>% st_buffer(1) ###buffer command expands outwards
view(the_buffer)
#View the result
mapview(the_buffer) + mapview(sf_GraingerHall, color = 'red')
###oval b/c geographically projected circle into a flat map... so show in google maps, it will look more like an even circle
```
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) ###converted to UTM, units in meters
