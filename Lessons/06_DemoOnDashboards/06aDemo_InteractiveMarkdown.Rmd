...
output: html_document #BEFORE: pdf_document, changed for Shiny Widget
runtime: shiny #add this for Shiny Widget, then run document to see 'knit' file
editor_options: 
...
selectInput(
  ...
  choice = c('tn_ug','tp_ug','nh34','no23','po4'),
  selected = 'no23')             #default choice "selected"

# Create a sliderInput to set the date range
sliderInput( ##add minimum and maximum output
  inputId = 'year_slider',
  label = 'select a start and end year',
  min = min(nutrient_data$year4), ##min of the input column, if a database is always updating
...
##react to our two widget inputs
renderPlot(
  plot_it(
    input$nutrient,                 ##"input" word associated with renderPlot
    input$year_slider[1],           ##first item
    input$year_slider[2],           ##second item
  )
)

renderText(
  paste("you selected",input$nutrient)
)
