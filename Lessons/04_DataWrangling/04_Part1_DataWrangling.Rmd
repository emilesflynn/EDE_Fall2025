...
> Add notes here: Import the dataset

```{r setup workspace, message = FALSE}
getwd #***Have the relative path
#install.packages(tidyverse) ***= collection of packages, ex. dplyr package
library(tidyverse)
#install.packages(lubridate) ***= create data options
library(lubridate)
#***import the data set
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) #***"StringAsFactors" = summary will work for character entries

#***dataframe functiosn to test if data was imported correctly
colnames(NTL.phys.data) #***all column names
head(NTL.phys.data) #***first few observations of the dataset
summary(NTL.phys.data) #***summary for each column
str(NTL.phys.data) #***what the data type is for each column
#notice that data is listed as a character type, we need to format as date
dim(NTL.phys.data)

class(NTL.phys.data$sampledate)
# Format sampledate as date, ***as.Date = provide the column we want and convert
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y")
#class(NTL.phys.data$sampledate)
#head(NTL.phys.data$sampledate) to check the first few observations
```

## Data Wrangling
>data wrangling is useful to clean, transform, explore, and analyze the data. 
...
```{r, results = "hide"}
#***dplyr = useful functions data manipulations, function works as verbs (ex. "filter")
vignette("dplyr") #provides help info
```
...
`>=`
`&`
>***multiple conditions to check, need to aggregate them
`|`
>*** "OR" if you want both of them to be true

```{r filtering data}
class(NTL.phys.data$lakeid) #***was a character type, now a factor
class(NTL.phys.data$depth)

# matrix filtering ***subset of filter data with rows with "0" for depth by row
#***regional data = NTL.phys.data
#***filter row for = depth
