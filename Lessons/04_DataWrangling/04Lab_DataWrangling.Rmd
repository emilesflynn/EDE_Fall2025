...
> Add notes here: Temperature, oxygen, and light are important factors in predicting the biotic/abiotic factors of the lake environment. For instance, there are certain species of fish that are more abundant in low dissolved oxygen and low temperature conditions. Meanwhile, these factors can also help track the nutrient content in lakes, for instance if there is low temperature and little/no oxygen available in a lake, then it may display signs of eutrophophication contributing to fish kills or high nutrient runoff in the surrounding area.Thus long-term data can provide detailed insights on the lake environment under certain conditions, thus aiding scientific hypothesis or guiding future research.
...
# dplyr filtering, ***depth column all have the value = 0
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0)

# Choose multiple conditions to filter ***only work with Paul+Peter lake
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#*** the comma is for "OR", if we had done ==paul lake and == peter lake then there would be zero results because only one name is given for each data
...
NTL.phys.data.year.1999 <- filter(NTL.phys.data, year4 == 1999)
#***pipe format
...
> Answer: They are always changing, therefore we should not wrangle data by row numbers.
