...
#refresh for new session, session page, restart R
```
...
```{r}
cars$speed #extract speeds column and shows in console
```
...
car_speed #shows speeds into R as a vector
```
...
```{r}
#Option 1
class(car_speed) #ask if dataframe

#Option 2
is.data.frame(car_speed) #ask again, tells you its numeric data
```
...
#Option 1 ## "cbind" (dataframe, vector), convert two dataframe to a vector
df <- cbind(df_car_speed,car_dist)  #similarly rows could be added using rbind()
class(df)
df

#convert 2 car vectors to a dataframe
...
```{r}
#useful function data.frame() ##name first column speed, second dist; create dataframe and show it
...
#you could also use this notation to delete columns w/ negatives
df <- df[,-2]
df
```
