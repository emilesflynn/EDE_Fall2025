...
##Look at the RainFall_BR data, the lubridate function will not process the "Statistics" column (values written as "average January")... therefore use the seperate function using the empty space between words
...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>%  ##Null2 = average, sep column to obtain a date object.
  mutate( Date = my(paste0(Month,"-",Year))) %>% #my "month year", copy/paste to a new column
  select(Date,Rainfall) ##slice data, creates a seperate table data file
...
##add the RainFall_BR_processed and Temp_BR_processed into one dataset with "inner join" function
...
##cannot have missing data or gaps in Time Series, check in Q5
...
Months <- as.data.frame(seq.Date(from=as.Date("1906/01/01"), 
                                 ...
                                 by="month")) ##by month
##create a sequence with all months from Jan 1906 to Dec 2016 (first and last day of my dataframe)

colnames(Months) <- "Date"
BR_complete <-left_join(Months, BR_complete) ##join with dataset BR_complete

summary(BR_complete) ##no change in number of observations, if change then that indicates missing data - identified by summary statistics with an "N/A" info
...
ggplot(BR_complete,
  ...
  labs (
    title = "Variations in Rainfall from 1906 to 2016"
  )
##very flat (unclear trend), seasonality
...
ggplot(BR_complete,
  ...
  geom_smooth(method="lm") + 
  labs(
    title = "Variations in Temperature from 1906 to 2016"
  )
...
##Time Series, start with specifying the start dates

f_month <-month(first(BR_complete$Date))
f_year <-year(first(BR_complete$Date))

BR_Rain_ts <-ts(BR_complete$Rainfall,
                frequency = 12, ##year = 1, daily = 365 (interval of seasonality)
                start = c(f_year, f_month))  
## specify if needed like (1901,1); we used the default (first year, first month) of the dataset
...
plot(BR_Temp_Decomp)
##shows that numbers increase in later years
...
##take time series and convert back to a dataset so that we can do linear analysis
BR_Rain_Components <- as.data.frame(BR_Rain_Decomp$time.series[,1:3] #want columns 1-3
...
##add trend component (red) to the rainfall plot
