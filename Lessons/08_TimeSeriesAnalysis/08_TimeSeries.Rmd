...
```{r}
##[CLASS NOTE] Bars tell you the concentration of the overall component, the trend is not a huge part of the dataset since the bar is narrow (zooms in since you cannot tell what the trend looks like in the seasonal graph. Remainder shows that most of the data is random, therefore the time series is not an ideal model for the data).
...
```{r}
##Date/Avg Wind Speed GRAPH: the middle of the frequencies showcase the trends of the data
## Monotonic Trend Analysis: seasonal historical data makes it hard to identify trends, therefore use the SeasonalMannKendell over using the Mann-Kendell functions, to see if there are trends in the seasons - not historical data
##Augmented Dickey Fuller: when linear models are not the best representation of the data (not typically used)
head(wind_data)
...
##[CLASS NOTES]~linear regression (variables), but use your own data
...
##[OVERALL CLASS NOTES] the more data, the better. If trying to predict the next year, you need multiple year data to model the seasonality and increase accuracy of the model. 
