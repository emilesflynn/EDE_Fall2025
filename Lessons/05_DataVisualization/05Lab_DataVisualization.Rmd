...
#^differenciates raw and output data
...
##^if using the newest version of R
##R currently updated its system and therefore the "expression" function does
##work the same, so the updated version (could work for R container due to older
##version)
...
```{r Legend Elements}
#Create a custom theme *part of assignment 5 hw
my_theme <- theme_base() + 
  theme(
    line = element_line(
      color='red',
...
##fun templates to try!!! (in own time)
my_theme <- theme_base()  + 
  theme(
    #line =              element_line(),
    #rect =              element_rect(),
...
surface_temp_by_depth <-PeterPaul.chem.nutrients %>%
  ggplot(aes(
    x = sampledate,
    y = temperature_C,
    color=depth)
