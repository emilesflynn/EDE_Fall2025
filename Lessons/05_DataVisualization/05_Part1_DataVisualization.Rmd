...
Effective data visualization depends on purposeful choices about graph types. The ideal graph type depends on the type of data and the message the visualizer desires to communicate. The best visualizations are clear and simple. A good resource for data visualization is [Data to Viz](https://www.data-to-viz.com/), which includes both a decision tree for visualization types and explanation pages for each type of data, including links to R resources to create them. Take a few minutes to explore this website.
 > Data to Viz: "ordered" plot ex = time
...
* geom_dotplot
* geom_density_ridges
* geom_point -- scattorplot
* geom_errorbar
* geom_smooth -- edit trendline
* geom_line 
* geom_area
* geom_abline (plus geom_hline and geom_vline)
* geom_text -- add/edit text labels

### Aesthetics
Here are some commonly used aesthetic types that can be manipulated in ggplot: 

* color
* fill - fill bar or point
* shape - point
* size - point
* transparency - point or bars

### Plotting continuous variables over time: Scatterplot and Line Plot

```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot
## (dataframe, aeasthetics, x label, y label) + type of graph
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point()

##generate same plot in different way
##add two plots on same window w/ print
O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot)
##now you get a row missing values error message in the console

# Fix this code
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone, color = "blue"))
print(O3plot2)
##shows up as red... why? Color is in rainbow color, so automatically recommends red, need to edit outside aethetics parenthesis. Rewrite as:
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)
...
##shape: changes shape of graph (now based on year, distribution of the points)
##color = site.name (assign color by site.name variable)
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)
##does tell you much about year... so seperate plots by years

# Separate plot with facets
##facet.wrap (one plot for each site rather than different colors) to clean up the data
PMplot.faceted <-
...
##subset: you can select the sites of interest, than seeing all of them
##facet_grid(site.name == rows, year = column)
PMplot.faceted2 <- 
...
##geom_line(): line point (to connect the points) need to change the y-axis
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line()
print(PMplot.line)
#get a time series plot
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO)
##there are two outliers on map, therefore need to adjust axis (since all clustered in bottom left)

# Adjust axes ##(min,max value ranges)
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
##scale_y_reverse: put zero on top
tempvsdepth <- 
...
##geom_smooth: (method line = "lm" linear), to add trendline of best fit
##add another line w/ geom_abline to customize trendline
NvsP <-
...
ggplot(df, aes(x = month_f, y = tp_ug)) +
  geom_boxplot()
Nutrientplot3 <-
...
##COLOR regards to the contour of the shape and FILL is the shape filling
print(Nutrientplot3)

# Dot plot
##geom_dotplot: see first, second, third of values
##binaxis (stack observatins over y), binwidth (set of beam), stackdir (stack in center position), position = dodge (no overlaps)
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
