...
Not all data is designed to have data scrapped, need to look at terms of service (will explicitly say "view only" or "do not scrape/take data"). URL/robots.txt says "disallow scrapping for csv"), example. https://nytimes.come/robots.txt and will describe subfolders and whether or not you can scrape data. Also, look for a way to directly download data OR an public API (like the census.gov site) with an R plug-in to easily extract data. Check the terms of service or robots.txt. Webpages with no API is only for viewing, because the website can get overloaded due to the risk of bots that target and increases traffic that eventually takes down sites. 
...
>>> browse water transformations, industrial water users, every month they report their daily withdrawal, discharge types = tables... cannot extract data (need to manually replicate OR use machine models). Point our code to a certain point and extract data.
...
#1. GRAB WEBSITE URL: Fetch the web resources from the URL
webpage <- read_html('https://www.ncwater.org/WUDC/app/WWATR/report/view/0004-0001/2020') ###list of two items with a set of nodes and documents
webpage

#### GOAL: create a dataframe with registrant, facility name, industry type, average water withdrawal
#2. Create a plug in. 
##FIRST (right click) "View Page Source", view raw source HTML  (Hypertecttext files interpreted by browser instructed on what/how to show it, read by a browser software like Chrome, want to extract individual bits)
##SECOND look for specific tags to pull specific values into our environment (head = attributes about webpage like title/javascript instructions, body = all contents of webpage, close)
```
...
>>> plug-in identifies tag (teach R to grab values as a variable based on tags), click on table cell boxes (desired only in green/yellow)
...
the_registrant <- webpage %>% 
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>%  ##use selector gadget tool and paste tag code name from webpage, html_nodes = look for all elements with these descriptors
  html_text()
the_registrant
```
